...
Scene::Scene()
{
	...
	selectedFunction = 0;
	dmcResolution = 1;
	bUseParallelRBF = true;  // Activé par défaut si OpenMP disponible
}
...
void Scene::render_gui()
{
	...
	// Option pour RBF parallèle (visible seulement si RBF est sélectionné)
	if (selectedFunction == 2) {
#if defined(_OPENMP) && defined(USE_PARALLEL_RBF)
		ImGui::Checkbox("Use OpenMP Parallel RBF", &bUseParallelRBF);
		if (ImGui::IsItemHovered())
			ImGui::SetTooltip("Parallelize matrix construction with OpenMP (faster)");
#else
		// OpenMP non disponible - afficher option grisée avec info
		bool tempDisabled = false;
		ImGui::PushItemFlag(ImGuiItemFlags_Disabled, true);
		ImGui::PushStyleVar(ImGuiStyleVar_Alpha, ImGui::GetStyle().Alpha * 0.5f);
		ImGui::Checkbox("Use OpenMP Parallel RBF (not available)", &tempDisabled);
		ImGui::PopStyleVar();
		ImGui::PopItemFlag();
		if (ImGui::IsItemHovered())
			ImGui::SetTooltip("OpenMP not compiled. Recompile with OpenMP support for parallelization.");
#endif
	}
...
void Scene::extractMesh()
{
	...
	case 1:
		...
		break;
	case 2:
		rbfFunction.init(&cloud, cloudRadius, 3.0f * cloudRadius, bUseParallelRBF);
