...
void Scene::render_gui()
{
	...
	ImGui::Spacing();
	ImGui::Checkbox("Show Constraints", &bShowConstraints);
	ImGui::Spacing();
	if(ImGui::Button("Apply##2"))
	{
		...
		{
		case 0:
			smoothing.globalLaplacian(constraints, bShowConstraints);
			break;
		case 1:
			smoothing.globalBilaplacian(constraints, constraintWeight, bShowConstraints);
			break;
		}
		mesh->sendToOpenGL(pointsProgram);
	}
